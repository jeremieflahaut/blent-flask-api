...
def test_orders_show_owner_can_view(client, client_user, make_token, make_order):
    order = make_order(client_user)
    token = make_token(client_user)
    ...
    assert "lignes" not in response.json
...
def test_orders_lines_requires_authentication(client, client_user, make_order):
    order = make_order(client_user)

    response = client.get(f"/api/commandes/{order.id}/lignes")
    assert response.status_code == 401


def test_orders_lines_owner_can_view(
    ...
    token = make_token(client_user)
    response = client.get(
        f"/api/commandes/{order.id}/lignes",
        headers={"Authorization": f"Bearer {token}"},
    )

    assert response.status_code == 200
    assert len(response.json) == 1
    assert response.json[0]["produit"]["id"] == products[0].id
    assert response.json[0]["quantite"] == 2
    assert response.json[0]["prix_unitaire"] == products[0].price_cents / 100


def test_orders_lines_forbidden_for_other_client(
    client, client_user, make_user, make_token, make_order
):
    other = make_user(email="other@example.net", name="Autre")
...
def test_orders_lines_admin_can_view_any(
    client, admin_user, client_user, make_token, make_order
):
    order = make_order(client_user)
...
def test_orders_lines_not_found(client, client_user, make_token):
    token = make_token(client_user)
    response = client.get(
